...
# -------- script para analise estatistica e metricas --------
import statistics

class MetricasExecucao:
    def __init__(self):
        self.avaliacoes_bl = 0
        self.verificacoes_nfp = 0
# -------- fim script para analise estatistica e metricas --------
...
# -------- script para metricas --------
def bottom_left_placement(permutation, instance, metricas=None):
# -------- fim script para metricas --------
    placed_items = []
    max_l_reached = 0
    ...
    for item in permutation:
        ...
        for rot_idx in range(item.num_rotations):
            ...
            for cx, cy in candidates:
                # -------- script para rastreamento no bl --------
                if metricas: metricas.avaliacoes_bl += 1
                # -------- fim script para rastreamento no bl --------
                ...
                if cy + box['w'] <= container_w:
                    ...
                    for p in placed_items:
                        # -------- script para rastreamento no nfp --------
                        if metricas: metricas.verificacoes_nfp += 1
                        # -------- fim script para rastreamento no nfp --------
...
# -------- script para metricas --------
def recozimento_simulado(instance, t0=1000, alpha=0.98, iter_max=100, metricas=None):
# -------- fim script para metricas --------
    ...
    # -------- script para repasse de metricas --------
    current_score, _, current_l = bottom_left_placement(current_order, instance, metricas)
    # -------- fim script para repasse de metricas --------
    ...
    while t > 0.1:
        for _ in range(iter_max):
            ...
            # -------- script para repasse de metricas --------
            new_score, _, new_l = bottom_left_placement(neighbor, instance, metricas)
            # -------- fim script para repasse de metricas --------
...
def main():
    ...
    for inst_nome in instancias_para_rodar:
        ...
        try:
            ...
            # -------- script para execucao multipla e estatistica --------
            SEEDS = [42 + i for i in range(30)]
            tempos_execucao = []
            l_resultados = []
            
            for s_idx, seed_atual in enumerate(SEEDS):
                random.seed(seed_atual)
                metricas = MetricasExecucao()
                
                start_time = time.time()
                best_order, final_l = recozimento_simulado(inst, t0=100, alpha=0.9, iter_max=300, metricas=metricas)
                _, final_placement, _ = bottom_left_placement(best_order, inst, metricas=metricas)
                ...
                l_resultados.append(final_l)
                
                with open(os.path.join(pasta_raiz, "resultados_detalhados_seeds.txt"), "a") as log:
                    log.write(f"Instancia={inst_nome} | Seed={seed_atual} | L={final_l:.2f} | "
                              f"Ocupacao={taxa_ocupacao:.2f}% | Avaliacoes_BL={metricas.avaliacoes_bl} | "
                              f"Checks_NFP={metricas.verificacoes_nfp} | Tempo={duracao:.4f}s\n")
                
                # Salvando as variáveis apenas da última execução para o plot_solution abaixo
            ...
            tempo_dp = statistics.stdev(tempos_execucao) if len(tempos_execucao) > 1 else 0.0
            l_medio = statistics.mean(l_resultados)

            print(f"    [OK LOTE DE 30] L Médio: {l_medio:.2f} | Tempo Médio: {tempo_medio:.2f}s (+-{tempo_dp:.2f}s)")
            
            with open(os.path.join(pasta_raiz, "resultados.txt"), "a") as log:
                log.write(f"{inst_nome} (MÉDIAS DE 30 SEEDS): L={l_medio:.2f}, Tempo={tempo_medio:.2f}s (+-{tempo_dp:.2f}s)\n")
            # -------- fim script para execucao multipla e estatistica --------
